...
class UADFVdataset(data.Dataset):
    ...
    def __init__(self,state,arbitrary_mask_percent=0,load_vis_img=False,label_transform=None,fraction=1.0,**args
        ...
        all_imgs = natsorted(glob.glob(osp.join(args['dataset_dir'], "UADFV-img", '*.png')))
        all_labels = natsorted(glob.glob(osp.join(args['dataset_dir'], "UADFV-mask/Overall_mask", '*.png')))
        
        if state == "train":
            self.imgs = all_imgs[0:1250]
            self.labels = all_labels[0:1250]

        elif state == "validation":
            self.imgs = all_imgs[1250:1350]
            self.labels = all_labels[1250:1350]
            
        else:
            self.imgs = all_imgs[1350:1450]
            self.labels = all_labels[1350:1450]
            

            self.ref_imgs = all_imgs[1450:1548]
            self.ref_labels = all_labels[1450:1548]
            

            self.ref_imgs= self.ref_imgs[:int(len(self.imgs)*self.fraction)]
            self.ref_labels= self.ref_labels[:int(len(self.labels)*self.fraction)]

            print("image 수:", len(self.imgs), self.imgs[0])
            print("label 수:", len(self.labels), self.labels[0])
